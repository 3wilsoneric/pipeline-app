...
Status: owner-fast-lane approved inherited-debt disposition; active machine overlay; not a replacement baseline
...
Machine-readable enforcement: `docs/refactoring/complexity-disposition.json`. Eric's 2026-09-07 owner-fast-lane authorization activates only the exact function and aggregate ceilings in that file; growth and unlisted failures remain red.

## Current result

The historical comparison remains above the reviewed baseline:
...
The owner approved the second path and then authorized `owner_fast_lane`. It does not excuse the debt or permit a new baseline. `npm run complexity:check` must confirm that the exact starting commit contains the same bounded failures and that the approved referral-store scope introduces no new failure or growth.

## Remaining start action

Run the machine ratchet at the exact starting commit and again on the candidate. Any unlisted failure, function growth, aggregate growth, or historical baseline rewrite blocks the slice. Independent review remains advisory under the current owner fast lane.
