# Referral preparation, first increment
...
Final focused run: 14 checks passed using `PIPELINE_NEXT_DIST_DIR=.next-preparation-check PIPELINE_E2E_PREBUILT=true PORT=3362 npx playwright test tests/e2e/assessment-preparation.spec.ts tests/e2e/emerald-surfaces.spec.ts --project=chromium --output=outputs/preparation-verified`. Production build, TypeScript, scoped ESLint, complexity check, and whitespace check passed. These are bounded product checks, not a refactor certification.
