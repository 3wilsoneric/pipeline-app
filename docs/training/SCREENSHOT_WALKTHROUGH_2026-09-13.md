...
### Release-environment sizing correction

Browser CI on combined candidate `4071cdec8852bef3db75ae4b472f2b40821c0260` exposed an 8px vertical overflow on slide 6 at 1280×720. It was reproduced in Linux Chromium from the cached Playwright `v1.61.1-noble` container: the article needed 599px but had 591px available. Images loaded successfully. Desktop outer padding is now 24px instead of 32px on each vertical edge; text sizes, image sizes, content, and navigation are unchanged. Mobile padding and scrolling remain unchanged.

The existing all-slide image-loading, desktop/mobile sizing, and keyboard-enlargement test passed in both Linux and macOS Chromium after the correction. Linux measurements show every slide fits its 591px article. Assertions now report the slide and actual dimensions, wait for fonts before measuring, and retain a screenshot of slide 6. The Linux browser used the local production build through a loopback-only test proxy; this is targeted cross-platform layout evidence, not another full CI or application certification. The application build/TypeScript, focused ESLint, and whitespace checks also passed.
