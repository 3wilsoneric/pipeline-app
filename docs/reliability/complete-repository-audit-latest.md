...
| `.github/workflows/refactor-agent.yml` | CI or supply-chain configuration | medium | 17611 B / 393 lines | Contains a hard-coded URL; verify environment portability, allowlisting, and that no endpoint is obsolete. | check:ci-impact + check:supply-chain + CI dry run |
...
| `docs/refactoring/CLOUD_REFACTOR_RUNBOOK.md` | Documentation or runbook | low | 5867 B / 99 lines | Check stale commands, obsolete architecture, contradictory sources of truth, missing owners, unsafe examples, and unverified recovery instructions. | link/config drift review against executable source |
