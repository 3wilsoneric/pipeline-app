...
| `.github/workflows/refactor-agent.yml` | CI or supply-chain configuration | medium | 16556 B / 371 lines | Contains a hard-coded URL; verify environment portability, allowlisting, and that no endpoint is obsolete. | check:ci-impact + check:supply-chain + CI dry run |
...
| `docs/refactoring/CLOUD_REFACTOR_RUNBOOK.md` | Documentation or runbook | low | 5140 B / 95 lines | Check stale commands, obsolete architecture, contradictory sources of truth, missing owners, unsafe examples, and unverified recovery instructions. | link/config drift review against executable source |
...
| `scripts/refactor-agent-control.mjs` | Operator, verification, or data script | low | 14275 B / 334 lines | Executes a subprocess; validate arguments, avoid shell interpolation, bound runtime, and propagate failures. | execute with sanitized fixture; verify dry-run/fail-closed behavior |
