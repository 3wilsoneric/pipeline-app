...
- Existing repository files: 590
- Tracked paths pending deletion: 0
- Risk classification: 45 critical, 127 high, 116 medium, 302 low
- Files with concrete static review flags: 129
...
- Review all 129 current static flags; this generator does not auto-dismiss findings from an older audit.
...
| @playwright/test | development | ^1.61.1 -> 1.61.1 | Apache-2.0 | Browser journey and accessibility testing | 24 | Skipped production-critical paths, shared test state, flaky timing assertions, and tests that pass only against warm caches. |
...
| eslint | development | ^9 -> 9.39.4 | MIT | Static code-quality enforcement | 8 | Rule drift, ignored directories, and warnings that do not fail CI. |
...
| lucide-react | runtime | ^0.577.0 -> 0.577.0 | ISC | Shared interface icons | 24 | Unlabelled icon-only controls, inconsistent sizing, and importing the full icon set into client bundles. |
| next | runtime | 16.2.11 -> 16.2.11 | MIT | Application framework and server runtime | 143 | Server/client boundary leaks, cache semantics, route-handler behavior, dynamic rendering drift, and framework-version API changes. |
...
| postcss | development | 8.5.26 -> 8.5.26 | MIT | CSS transformation runtime | 7 | Plugin ordering, parser vulnerabilities, and output differences between local and CI builds. |
| postgres | runtime | ^3.4.9 -> 3.4.9 | Unlicense | PostgreSQL client | 82 | Pool exhaustion, missing transaction boundaries, unsafe dynamic SQL, statement timeouts, retrying non-idempotent writes, and connection leaks. |
| react | runtime | 19.2.4 -> 19.2.4 | MIT | User-interface runtime | 41 | Effect races, stale closures, unstable keys, hydration mismatches, and state duplicated across server/client boundaries. |
...
| server-only | runtime | ^0.0.1 -> 0.0.1 | MIT | Build-time server-boundary guard | 77 | Missing imports in credential/data modules and false confidence when transitive client imports bypass the intended boundary. |
...
| typescript | development | ^5 -> 5.9.3 | Apache-2.0 | Static type checking and compiler APIs | 16 | Compiler-version drift, skipped checks, broad assertions, and emitted/runtime behavior assumed from types. |
...
| `.github/workflows/ci.yml` | CI or supply-chain configuration | medium | 10308 B / 255 lines | Contains a hard-coded URL; verify environment portability, allowlisting, and that no endpoint is obsolete. | check:ci-impact + check:supply-chain + CI dry run |
...
| `app/(pipeline)/academy/page.tsx` | Next.js route or application shell | low | 1228 B / 43 lines | Check server/client boundaries, loading/error/empty states, navigation persistence, metadata, and hydration behavior. | typecheck + lint + build + responsive/accessibility E2E |
...
| `components/pipeline/AssessmentWorkspace.tsx` | Pipeline user interface | medium | 86019 B / 1600 lines | Large module (1600 lines): split by behavior before adding more responsibilities. | typecheck + lint + build + responsive/accessibility E2E |
...
| `components/pipeline/OperationsDashboard.tsx` | Pipeline user interface | medium | 22268 B / 503 lines | Check duplicated state, autosave/conflicts, focus and labels, responsive overflow, stale data, and recoverable actions. | typecheck + lint + build + responsive/accessibility E2E |
...
| `components/pipeline/PipelineActionNav.tsx` | Pipeline user interface | medium | 6779 B / 172 lines | Check duplicated state, autosave/conflicts, focus and labels, responsive overflow, stale data, and recoverable actions. | typecheck + lint + build + responsive/accessibility E2E |
| `components/pipeline/PipelineAppShell.tsx` | Pipeline user interface | medium | 984 B / 28 lines | Check duplicated state, autosave/conflicts, focus and labels, responsive overflow, stale data, and recoverable actions. | typecheck + lint + build + responsive/accessibility E2E |
| `components/pipeline/PipelineCalendar.tsx` | Pipeline user interface | medium | 23641 B / 398 lines | Uses timers in UI/runtime code; verify cleanup, visibility behavior, backoff, and no duplicate pollers. | typecheck + lint + build + responsive/accessibility E2E |
| `components/pipeline/PipelineDeveloperAcademy.tsx` | Pipeline user interface | medium | 26768 B / 605 lines | Uses browser storage; verify no PHI/tokens, user scoping, expiry, and cross-session cleanup. | typecheck + lint + build + responsive/accessibility E2E |
| `components/pipeline/PipelineHeader.tsx` | Pipeline user interface | medium | 12474 B / 252 lines | Check duplicated state, autosave/conflicts, focus and labels, responsive overflow, stale data, and recoverable actions. | typecheck + lint + build + responsive/accessibility E2E |
...
| `components/pipeline/PipelineWelcome.tsx` | Pipeline user interface | medium | 19048 B / 476 lines | Uses timers in UI/runtime code; verify cleanup, visibility behavior, backoff, and no duplicate pollers. | typecheck + lint + build + responsive/accessibility E2E |
...
| `components/pipeline/ReferralHome.tsx` | Pipeline user interface | medium | 51673 B / 1085 lines | Large module (1085 lines): split by behavior before adding more responsibilities. | typecheck + lint + build + responsive/accessibility E2E |
| `components/pipeline/ReferralPacketCanvas.tsx` | Pipeline user interface | medium | 108818 B / 2646 lines | Large module (2646 lines): split by behavior before adding more responsibilities. | typecheck + lint + build + responsive/accessibility E2E |
...
| `components/pipeline/ReferralWorklist.tsx` | Pipeline user interface | medium | 9363 B / 208 lines | Check duplicated state, autosave/conflicts, focus and labels, responsive overflow, stale data, and recoverable actions. | typecheck + lint + build + responsive/accessibility E2E |
...
| `database/migration-checksums.json` | Database fixture or manifest | low | 1857 B / 24 lines | Check ownership, stale duplication, unsafe defaults, error handling, and whether an executable test covers the file's behavior. | check:platform:fast and owning feature test |
...
| `docs/FILE_MAP.md` | Documentation or runbook | low | 18273 B / 171 lines | Check stale commands, obsolete architecture, contradictory sources of truth, missing owners, unsafe examples, and unverified recovery instructions. | link/config drift review against executable source |
...
| `docs/academy/ARCHITECTURE_GUIDE.md` | Documentation or runbook | low | 10182 B / 174 lines | Check stale commands, obsolete architecture, contradictory sources of truth, missing owners, unsafe examples, and unverified recovery instructions. | link/config drift review against executable source |
| `docs/academy/CHANGE_DOSSIER_TEMPLATE.md` | Documentation or runbook | low | 1826 B / 85 lines | Check stale commands, obsolete architecture, contradictory sources of truth, missing owners, unsafe examples, and unverified recovery instructions. | link/config drift review against executable source |
| `docs/academy/CURRICULUM.md` | Documentation or runbook | low | 4834 B / 78 lines | Check stale commands, obsolete architecture, contradictory sources of truth, missing owners, unsafe examples, and unverified recovery instructions. | link/config drift review against executable source |
| `docs/academy/README.md` | Documentation or runbook | low | 5262 B / 96 lines | Check stale commands, obsolete architecture, contradictory sources of truth, missing owners, unsafe examples, and unverified recovery instructions. | link/config drift review against executable source |
| `docs/academy/WORKBOOK.md` | Documentation or runbook | low | 3653 B / 138 lines | Check stale commands, obsolete architecture, contradictory sources of truth, missing owners, unsafe examples, and unverified recovery instructions. | link/config drift review against executable source |
| `docs/academy/academy-registry.json` | Documentation or runbook | low | 8685 B / 235 lines | Check stale commands, obsolete architecture, contradictory sources of truth, missing owners, unsafe examples, and unverified recovery instructions. | link/config drift review against executable source |
| `docs/academy/lessons/01_CREATE_REFERRAL.md` | Documentation or runbook | low | 13696 B / 270 lines | Check stale commands, obsolete architecture, contradictory sources of truth, missing owners, unsafe examples, and unverified recovery instructions. | link/config drift review against executable source |
...
| `docs/refactoring/CYCLOMATIC_COMPLEXITY_POLICY.md` | Documentation or runbook | low | 3200 B / 68 lines | Check stale commands, obsolete architecture, contradictory sources of truth, missing owners, unsafe examples, and unverified recovery instructions. | link/config drift review against executable source |
...
| `docs/refactoring/README.md` | Documentation or runbook | low | 5128 B / 101 lines | Check stale commands, obsolete architecture, contradictory sources of truth, missing owners, unsafe examples, and unverified recovery instructions. | link/config drift review against executable source |
...
| `docs/reliability/complete-repository-audit-latest.md` | Generated audit evidence | medium | 263936 B / 1367 lines | Do not edit manually; regenerate and verify that inventory counts match the current worktree. | regenerate with npm run audit:repository |
...
| `docs/reliability/dependency-inventory.json` | Generated audit evidence | medium | 201476 B / 7238 lines | Do not edit manually; regenerate and verify that inventory counts match the current worktree. | regenerate with npm run audit:repository |
...
| `docs/reliability/repository-file-inventory.json` | Generated audit evidence | medium | 333220 B / 6719 lines | Do not edit manually; regenerate and verify that inventory counts match the current worktree. | regenerate with npm run audit:repository |
...
| `lib/academy/academy-course.ts` | Shared server/domain library | low | 39380 B / 800 lines | Check ownership, stale duplication, unsafe defaults, error handling, and whether an executable test covers the file's behavior. | check:platform:fast and owning feature test |
...
| `package.json` | Build, runtime, or repository configuration | low | 12661 B / 188 lines | Check local/CI/production parity, secret handling, ignored outputs, runtime version pinning, bundle boundaries, and deployment defaults. | check:platform:fast and owning feature test |
...
| `scripts/academy-readiness.mjs` | Operator, verification, or data script | low | 8364 B / 238 lines | Check destructive confirmation, dry-run mode, bounded inputs, deterministic exit codes, resumability, secret/PHI-safe output, and partial-failure recovery. | execute with sanitized fixture; verify dry-run/fail-closed behavior |
| `scripts/academy-route-contracts.mjs` | Operator, verification, or data script | low | 4698 B / 123 lines | Uses browser storage; verify no PHI/tokens, user scoping, expiry, and cross-session cleanup. | execute with sanitized fixture; verify dry-run/fail-closed behavior |
...
| `scripts/assessor-workflow-contracts.mjs` | Operator, verification, or data script | low | 14621 B / 173 lines | Contains a hard-coded URL; verify environment portability, allowlisting, and that no endpoint is obsolete. | execute with sanitized fixture; verify dry-run/fail-closed behavior |
...
| `scripts/code-quality-readiness.mjs` | Operator, verification, or data script | low | 19678 B / 303 lines | Executes a subprocess; validate arguments, avoid shell interpolation, bound runtime, and propagate failures. | execute with sanitized fixture; verify dry-run/fail-closed behavior |
...
| `scripts/database-readiness.mjs` | Operator, verification, or data script | high | 27546 B / 361 lines | Uses SELECT *; verify response growth, schema coupling, and PHI minimization. | execute with sanitized fixture; verify dry-run/fail-closed behavior |
| `scripts/database-restore-verify.mjs` | Operator, verification, or data script | high | 5315 B / 129 lines | Check destructive confirmation, dry-run mode, bounded inputs, deterministic exit codes, resumability, secret/PHI-safe output, and partial-failure recovery. | execute with sanitized fixture; verify dry-run/fail-closed behavior |
| `scripts/database-rollback-drill.mjs` | Operator, verification, or data script | high | 26293 B / 384 lines | Explicitly suppresses a rejected promise; verify this is genuinely optional and observable. | execute with sanitized fixture; verify dry-run/fail-closed behavior |
...
| `scripts/postgres-concurrency-certification.mjs` | Operator, verification, or data script | low | 17349 B / 412 lines | Contains destructive behavior; require dry-run, explicit scope, and recovery evidence. | execute with sanitized fixture; verify dry-run/fail-closed behavior |
| `scripts/postgres-integration-fixtures.mjs` | Operator, verification, or data script | low | 10612 B / 188 lines | Contains destructive behavior; require dry-run, explicit scope, and recovery evidence. | execute with sanitized fixture; verify dry-run/fail-closed behavior |
| `scripts/postgres-integrity-audit.mjs` | Operator, verification, or data script | low | 5944 B / 142 lines | Check destructive confirmation, dry-run mode, bounded inputs, deterministic exit codes, resumability, secret/PHI-safe output, and partial-failure recovery. | execute with sanitized fixture; verify dry-run/fail-closed behavior |
| `scripts/postgres-live-smoke.mjs` | Operator, verification, or data script | low | 9239 B / 206 lines | Check destructive confirmation, dry-run mode, bounded inputs, deterministic exit codes, resumability, secret/PHI-safe output, and partial-failure recovery. | execute with sanitized fixture; verify dry-run/fail-closed behavior |
...
| `scripts/refactor-setup-readiness.mjs` | Operator, verification, or data script | low | 6573 B / 115 lines | Check destructive confirmation, dry-run mode, bounded inputs, deterministic exit codes, resumability, secret/PHI-safe output, and partial-failure recovery. | execute with sanitized fixture; verify dry-run/fail-closed behavior |
...
| `scripts/seed-production-reference-data.mjs` | Operator, verification, or data script | high | 2294 B / 48 lines | Check destructive confirmation, dry-run mode, bounded inputs, deterministic exit codes, resumability, secret/PHI-safe output, and partial-failure recovery. | execute with sanitized fixture; verify dry-run/fail-closed behavior |
...
| `tests/e2e/academy.spec.ts` | Browser journey test or snapshot | low | 3458 B / 64 lines | Uses browser storage; verify no PHI/tokens, user scoping, expiry, and cross-session cleanup. | run owning Playwright project and review skips/snapshots |
...
| `tests/e2e/pipeline-smoke.spec.ts` | Browser journey test or snapshot | medium | 137123 B / 2731 lines | Large module (2731 lines): split by behavior before adding more responsibilities. | run owning Playwright project and review skips/snapshots |
...
| `tests/e2e/visual-regression.spec.ts-snapshots/mobile-new-packet-darwin.png` | Browser journey test or snapshot | low | 30692 B / binary | Check deterministic isolation, meaningful assertions, authenticated roles, mobile/desktop parity, concurrency, recovery, and skipped-path justification. | run owning Playwright project and review skips/snapshots |
| `tests/e2e/visual-regression.spec.ts-snapshots/mobile-new-packet-linux.png` | Browser journey test or snapshot | low | 28417 B / binary | Check deterministic isolation, meaningful assertions, authenticated roles, mobile/desktop parity, concurrency, recovery, and skipped-path justification. | run owning Playwright project and review skips/snapshots |
...
- `components/pipeline/PipelineDeveloperAcademy.tsx`: Uses browser storage; verify no PHI/tokens, user scoping, expiry, and cross-session cleanup.
...
- `components/pipeline/ReferralHome.tsx`: Large module (1085 lines): split by behavior before adding more responsibilities. Uses timers in UI/runtime code; verify cleanup, visibility behavior, backoff, and no duplicate pollers.
- `components/pipeline/ReferralPacketCanvas.tsx`: Large module (2646 lines): split by behavior before adding more responsibilities. Uses timers in UI/runtime code; verify cleanup, visibility behavior, backoff, and no duplicate pollers. Uses browser storage; verify no PHI/tokens, user scoping, expiry, and cross-session cleanup. Explicitly suppresses a rejected promise; verify this is genuinely optional and observable.
...
- `tests/e2e/pipeline-smoke.spec.ts`: Large module (2731 lines): split by behavior before adding more responsibilities. Uses browser storage; verify no PHI/tokens, user scoping, expiry, and cross-session cleanup.
