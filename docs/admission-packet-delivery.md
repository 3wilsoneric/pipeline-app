...
Meet the Client includes the saved message, every available uploaded workspace file, and the generated client data sheet. New assessor handoffs save these as ordinary attachments directly in the assessor’s Outlook Drafts. The assessor sends from Outlook, and recipients open the attached files without a code, download portal or Pipeline account. See [Outlook Drafts setup](outlook-handoff-setup.md) for the current flow, recovery and activation requirements. **Not production yet — no draft is created and no email is sent until the owner activates it.**

The link behavior below applies only to existing linked packets and the legacy service-sender endpoint. That endpoint uses a link above 50 files or 20 MiB, when direct attachment permissions are missing, or after a definite provider size refusal. New Outlook drafts do not use this fallback. Preserve legacy access records and routes for previously issued links.
...
## Legacy linked-packet experience
...
- `npm run check:meet-client-delivery` also covers ordinary Outlook attachments, exact byte verification, duplicate filenames, large chunk uploads, lost-response recovery, provider refusals, pinned sources, same-mailbox authorization, operation leases and the production hold. Attachment-only records cannot use recipient codes or legacy download controls; both local and PostgreSQL adapters exercise that boundary.
