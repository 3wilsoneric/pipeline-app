...
export default function ReferralIntakeSummary({ referral, headerActions, onEditField }: {
  ...
  const fact = (label: string, key: PersistedCanvasFieldKey, wide = false): ClientChartFact => {
    ...
    return { label, value: (key === "name" ? presentClientName(value) : key === "dob" || key === "referralReceived" ? formatProfileDate(value) : value)?.trim() || "Not documented", ...(wide ? { span: "wide" } : {}) };
  };
  ...
  return <ClientChartFrame label="Referral chart">
    ...
    <ChartBand title="Referral details">
      <ChartGrid ariaLabel="Referral details" columns="care">
        {[
          fact("Assessor", "owner"), fact("Referral received", "referralReceived"),
