...
function AppNavigation({ focused, searchOpen }: { focused: boolean; searchOpen: boolean }) {
  ...
  return (
    <div data-assessment-app-navigation={focused ? "collapsed" : "standard"} className={`relative shrink-0 ${focused ? "z-[100] h-0" : "z-10"}`}
      ...
      {focused ? <button type="button" aria-label="Show app navigation" title="App navigation" aria-expanded={open || searchOpen} aria-controls="pipeline-app-navigation" onClick={() => setOpen(true)} className="absolute left-2 top-1.5 z-20 flex h-11 w-11 items-center justify-center rounded bg-white text-[#315d41] hover:bg-[#edf3f2] focus-visible:outline-2 focus-visible:outline-[#0f8b73]"><ChevronDown size={16} aria-hidden="true" /></button> : null}
      {focused ? <div data-assessment-nav-edge aria-hidden="true" className="absolute inset-x-0 top-0 h-1" /> : null}
