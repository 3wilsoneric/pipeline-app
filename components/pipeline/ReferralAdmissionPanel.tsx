...
export default function ReferralAdmissionPanel({ referral, packetSentAt, admissionDate, disabled, onAdmissionDateChange, onSaveAdmissionDate }: Props) {
  ...
  return <section aria-label="Admission follow-through" className="mb-4 space-y-4 border-b border-[#e3e6e4] pb-4">
    ...
    <p id="planned-admission-help" className="text-[11px] text-[#68716c]">{packetSentAt ? "The packet has been sent. Reviewing it or changing the planned date does not send another email." : "Next: set the planned date, then review the email and packet. You can preview without a date. Nothing sends until you confirm Send."}</p>
