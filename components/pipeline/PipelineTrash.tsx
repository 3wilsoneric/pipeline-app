...
export default function PipelineTrash() {
  ...
  return (
    <main data-performance-ready={pipelineSurfaceReady("trash", loading, error)} aria-busy={loading} className="h-full overflow-y-auto pipeline-page-surface px-4 pb-8 pt-2 sm:px-6 lg:px-8">
          ...
          <div className="mt-3 divide-y divide-[#d9d9d9] border-y border-[#d9d9d9]">
            {referrals.map((referral) => (
              <div key={referral.id} className="grid gap-3 px-3 py-4 sm:grid-cols-[minmax(0,1fr)_180px_140px_auto] sm:items-center">
                <div className="min-w-0"><div className="truncate text-[13px] font-bold text-[#111111]" title={formatClientIdentityTitle(referral)}>{formatClientIdentityTitle(referral)}</div>{formatClientIdentityDetail(resolveClientGender(referral.gender), resolveClientCommunity(referral.community), referral.owner || "Unassigned") ? <div className="mt-1 truncate text-[10px] text-[#737373]">{formatClientIdentityDetail(resolveClientGender(referral.gender), resolveClientCommunity(referral.community), referral.owner || "Unassigned")}</div> : null}</div>
