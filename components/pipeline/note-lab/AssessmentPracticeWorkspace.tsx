        ...
        <div className="min-h-0 flex-1 border-x border-b border-[#cfd8d4] bg-white lg:grid lg:grid-cols-[230px_minmax(0,1fr)]">
                    ...
                    {group.sections.map((sectionKey) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex w-full items-center justify-between gap-3 border-l-2 px-3 py-2.5 text-left text-[11px] font-black transition-colors ${active ? "border-[#0f8b73] bg-[#e7f3ee] text-[#0f6f5d]" : "border-transparent text-[#595959] hover:bg-white hover:text-[#0f8b73]"}`}
                        >
          ...
          <main data-assessment-practice-scroll className="min-h-0 overflow-y-auto bg-white">
            ...
            <div className="mx-auto max-w-[980px] px-5 py-7 sm:px-8">
              ...
              <PracticeQuestions questions={questions} data={data} requiredFields={requiredFields} onUpdate={update} />

              <footer className="mt-7 flex items-center justify-between gap-3">
                <button type="button" disabled={currentIndex === 0} onClick={() => move(-1)} className="flex h-10 items-center gap-2 border border-[#c9ceca] px-4 text-[11px] font-black text-[#4d5652] hover:border-[#0f8b73] hover:text-[#0f8b73] disabled:opacity-35"><ChevronLeft size={14} />Previous</button>
                <button type="button" disabled={currentIndex === assessmentInterviewSections.length - 1} onClick={() => move(1)} className="flex h-10 items-center gap-2 bg-[#111111] px-4 text-[11px] font-black text-white hover:bg-[#0f8b73] disabled:opacity-35">Next section<ChevronRight size={14} /></button>
              </footer>
            </div>
          </main>
        </div>
...
function PracticeQuestions({ questions, data, requiredFields, onUpdate }: {
...
function PracticeField({ question, value, unableReason, required, onUnableReasonChange, onUpdate }: {
  ...
  return (
    <div data-guide-target={practiceGuideTarget(question.field)} className={question.span === "full" || question.control === "multi_select" ? "md:col-span-2" : ""}>
      <div className="mb-1.5 flex items-center justify-between gap-2">
        <label htmlFor={id} className="text-[11px] font-black text-[#444444]">{assessmentInterviewFieldLabel(question.field)}{required ? " *" : ""}</label>
        {hasAssessmentInterviewValue(value) ? <Check size={12} className="text-[#0f8b73]" /> : required ? <span className="text-[9px] font-semibold uppercase text-[#9a6115]">Required</span> : <span className="text-[9px] font-semibold uppercase text-[#999999]">Optional</span>}
      </div>
      ...
      {question.control === "textarea" ? <WritingExample field={question.field} /> : null}
    </div>
  );
}
...
function WritingExample({ field }: { field: AssessmentToolFieldKey }) {
  ...
  return (
    <details className="mt-2 border border-[#d9dfdb] bg-[#f8faf9]">
      <summary className="flex min-h-9 cursor-pointer list-none items-center justify-between gap-3 px-3 py-2 marker:hidden">
        <span className="text-[10px] font-black text-[#315e50]">Example</span>
        <span className="text-[9px] font-semibold text-[#7b837e]">{specification.formatLabel}</span>
      </summary>
      <div className="border-t border-[#d9dfdb] bg-white px-3 py-3">
        <div className="text-[9px] font-black uppercase tracking-[0.08em] text-[#315e50]">Structure</div>
        <p className="mt-1 text-[11px] font-semibold leading-5 text-[#3f4a45]">{specification.formatTemplate}</p>
        <div className="mt-3 text-[9px] font-black uppercase tracking-[0.08em] text-[#595959]">Good note</div>
        <p className="mt-1 text-[11px] leading-5 text-[#4f5652]">{specification.strongExample}</p>
      </div>
    </details>
  );
}
