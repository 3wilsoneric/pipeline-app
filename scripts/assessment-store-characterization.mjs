...
if (workingTreeChanges) fail("Commit or remove working-tree changes before capturing assessment characterization evidence.");

const startedAt = new Date();
const capturedFromCommit = execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim();
const testPath = "tests/e2e/operational/assessment-store-characterization.spec.ts";
const tempRoot = mkdtempSync(path.join(os.tmpdir(), "pipeline-assessment-characterization-"));
const databaseName = `pipeline_assessment_characterization_${process.pid}_${randomUUID().slice(0, 8).replaceAll("-", "")}`;
...
try {
  ...
  failureMessage = error instanceof Error ? error.message : "Assessment-store characterization failed.";
} finally {
  if (databaseCreated) {
...
const payload = {
  ...
  profile: "assessment_store_characterization",
  ...
  normalized_nondeterminism: [
    "Store-generated referral, assessment, audit, and addendum identifiers are asserted within an adapter, not compared across adapters.",
    "Storage timestamps are checked for lifecycle ordering and immutability, not equality across adapters.",
  ],
...
async function runCharacterization(adapter, port, extraEnv) {
  const result = await runCommand(adapter, path.join(process.cwd(), "node_modules", ".bin", "playwright"), [
    ...
    PIPELINE_ASSESSMENT_CHARACTERIZATION: "true",
    PORT: String(port),
    ...extraEnv,
  }, true);
  return {
...
function writeArtifact(value) {
  ...
  const target = path.join(directory, `assessment-store-${timestamp}.json`);
  writeFileSync(target, `${JSON.stringify(value, null, 2)}\n`, { encoding: "utf8", mode: 0o600 });
  writeFileSync(path.join(directory, "assessment-store-latest.json"), `${JSON.stringify(value, null, 2)}\n`, { encoding: "utf8", mode: 0o600 });
  return target;
}
