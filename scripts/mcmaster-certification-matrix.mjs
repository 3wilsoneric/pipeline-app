...
const evidence = [
  ...
  item("clinical.profile", "Governed Alamo client directory and profile are verified", "tests/e2e/pipeline-home.spec.ts", "opens the Alamo enhanced client directory and governed profile"),
  item("clinical.failure", "A temporary profile failure has an explicit recovery path", "tests/e2e/pipeline-home.spec.ts", "recovers a client profile after a temporary server failure"),
  ...
  item("capacity.collaboration", "Twenty simultaneous identities exercise presence, polling, writes, and conflicts under a p95 budget", "scripts/collaboration-load-smoke.mjs", ["PIPELINE_COLLABORATION_USERS", "PIPELINE_COLLABORATION_P95_LIMIT_MS"]),
  item("capacity.soak", "A bounded aggregate-only soak detects latency drift and event-loop pressure", "scripts/http-soak-smoke.mjs", ["final_phase_drift", "monitorEventLoopDelay", "172_800"]),
  item("capacity.complete", "One command composes browser, API, collaboration, scale, soak, and gated external evidence", "scripts/complete-performance-certification.mjs", ["browser_performance", "http_capacity", "collaboration_load", "bounded_soak", "postgres_database_capacity"]),
