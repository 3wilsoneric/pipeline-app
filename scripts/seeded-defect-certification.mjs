...
const mutants = [
  mutant("WF-01", "workflow incorrectly blocks beginning an assessment from new work", "lib/pipeline/referral-workflow.ts",
    'return boardStages.filter((target) => target !== stage);', 'return boardStages.filter((target) => target !== stage && !(stage === "New" && target === "Assessment"));'),
  mutant("WF-02", "accepted referral incorrectly prevents revisiting earlier work", "lib/pipeline/referral-workflow.ts",
    'return boardStages.filter((target) => target !== stage);', 'return boardStages.filter((target) => target !== stage && stage !== "Accepted / Admitted");'),
  mutant("WF-03", "missing owner alert is hidden", "lib/pipeline/referral-workflow.ts",
    'return isUnassignedOwner(referral.owner)', 'return false'),
  mutant("WF-04", "missing packet alert is hidden", "lib/pipeline/referral-workflow.ts",
    'return !hasInitialPacket(referral)', 'return false'),
  mutant("WF-05", "unreviewed packet alert is hidden", "lib/pipeline/referral-workflow.ts",
    'return !isPacketReviewed(referral)', 'return false'),
  mutant("WF-06", "incomplete assessment alert is hidden", "lib/pipeline/referral-workflow.ts",
    'return !isAssessmentComplete(referral, context)', 'return false'),
  ...
  mutant("WF-08", "move-in blockers are ignored", "lib/pipeline/referral-workflow.ts",
    '["admission_decision", "move_in"],', '["admission_decision"],'),
  ...
  mutant("WF-11", "admission-decision requirements are ignored", "lib/pipeline/referral-workflow.ts",
    '["admission_decision", "move_in"],', '["move_in"],'),
  ...
  mutant("AS-01", "signed assessment projection ignores the immutable signature", "lib/assessment/assessment-lifecycle-validation.ts",
    'if (assessment.signed_at) return "assessment_signed";', 'if (false && assessment.signed_at) return "assessment_signed";'),
  mutant("AS-02", "cancelled assessment remains incorrectly scheduled", "lib/assessment/assessment-lifecycle-validation.ts",
    'if (action === "assessment_cancelled" || action === "assessment_no_show") return "ready_to_schedule";', 'if (action === "assessment_no_show") return "ready_to_schedule";'),
  mutant("AS-03", "legacy completed assessment loses ready-to-sign compatibility", "lib/assessment/assessment-lifecycle-validation.ts",
    'if (assessment.status === "complete") return "assessment_ready_to_sign";', 'if (false && assessment.status === "complete") return "assessment_ready_to_sign";'),
  mutant("AS-04", "pending extracted assessment evidence is treated as reviewed", "lib/assessment/assessment-lifecycle-validation.ts",
  ...
  mutant("WR-04", "candidate evidence traversal key is accepted", "lib/extraction/worker-report-validation.ts",
    'if (candidate.evidence_blob_key) safeBlobKey(candidate.evidence_blob_key);', 'void candidate.evidence_blob_key;'),
  mutant("WR-05", "reversed evidence bounding box is accepted", "lib/extraction/worker-report-validation.ts",
    'if (value[0] >= value[2] || value[1] >= value[3])', 'if (false && (value[0] >= value[2] || value[1] >= value[3]))'),
  mutant("EX-06", "provider attachment can update a reclaimed attempt", "lib/extraction/processing-worker.ts",
    'provider_job_id = ${run.job_run_id}, provider_state = \'queued\',\n          heartbeat_at = now(), lease_expires_at = now() + interval \'30 minutes\', updated_at = now()\n        where extraction_job_id = ${job.extraction_job_id}::uuid and status = \'running\'\n          and attempt_count = ${job.attempt_count} and attempt_token = ${job.attempt_token}::uuid',
    'provider_job_id = ${run.job_run_id}, provider_state = \'queued\',\n          heartbeat_at = now(), lease_expires_at = now() + interval \'30 minutes\', updated_at = now()\n        where extraction_job_id = ${job.extraction_job_id}::uuid and status = \'running\''),
  mutant("EX-07", "provider reconciliation can update a newer attempt", "lib/extraction/processing-worker.ts",
    'where extraction_job_id = ${job.extraction_job_id}::uuid and status = \'running\'\n      and attempt_count = ${job.attempt_count} and attempt_token = ${job.attempt_token}::uuid\n      and provider_job_id = ${job.provider_job_id}',
    'where extraction_job_id = ${job.extraction_job_id}::uuid and status = \'running\'\n      and provider_job_id = ${job.provider_job_id}'),
  mutant("EX-08", "unsafe document state can be hidden by stale extracted fields", "lib/extraction/processing-worker.ts",
    'Number(state.unsafe_document_count) > 0 || Number(state.failed_count) > 0',
