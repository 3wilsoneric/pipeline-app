...
check("scenario library covers every role and has one safe answer per case", resources.operatorScenarios.length >= 12 && [["admin", 2], ["assessment_coordinator", 2], ["reviewer", 2], ["viewer", 1]].every(([role, minimum]) => resources.scenariosForRole(role).length >= minimum) && resources.operatorScenarios.every((scenario) => scenario.choices.filter((choice) => choice.safe).length === 1 && scenario.moduleIds.every((id) => curriculum.operatorModuleIds.includes(id))));
