...
if (workingTreeChanges) fail("Commit or remove working-tree changes before capturing refactor characterization evidence.");

const startedAt = new Date();
const capturedFromCommit = execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim();
const testPath = "tests/e2e/operational/referral-store-characterization.spec.ts";
const tempRoot = mkdtempSync(path.join(os.tmpdir(), "pipeline-referral-characterization-"));
const databaseName = `pipeline_referral_characterization_${process.pid}_${randomUUID().slice(0, 8).replaceAll("-", "")}`;
...
try {
  ...
  failureMessage = error instanceof Error ? error.message : "Referral-store characterization failed.";
} finally {
  if (databaseCreated) {
...
const payload = {
  ...
  profile: "referral_store_characterization",
...
async function runCharacterization(adapter, port, extraEnv) {
  const result = await runCommand(adapter, path.join(process.cwd(), "node_modules", ".bin", "playwright"), [
    ...
    PIPELINE_REFERRAL_CHARACTERIZATION: "true",
    PORT: String(port),
    ...extraEnv,
  }, true);
  return {
...
function writeArtifact(value) {
  ...
  const target = path.join(directory, `referral-store-${timestamp}.json`);
  writeFileSync(target, `${JSON.stringify(value, null, 2)}\n`, { encoding: "utf8", mode: 0o600 });
  writeFileSync(path.join(directory, "referral-store-latest.json"), `${JSON.stringify(value, null, 2)}\n`, { encoding: "utf8", mode: 0o600 });
  return target;
}
