...
const workspaceMembers = read("lib/pipeline/workspace-members.ts");
...
const importedWorkspaceLifecycleRollback = read("database/rollbacks/0026_imported_workspace_lifecycle.sql");
...
const extractionEvidenceBoundingBoxesRollback = read("database/rollbacks/0032_extraction_evidence_bounding_boxes.sql");
const workflowContinuityRollback = read("database/rollbacks/0033_workflow_continuity.sql");
const contactDirectoryRollback = read("database/rollbacks/0034_contact_directory.sql");
...
check(
  "import provenance remains explicit while active work uses lifecycle status",
...
check(
  "imported workspace ingestion has an idempotent batch ledger",
...
check("import provenance is independent from active and completed workspace state", importedWorkspaceLifecycleMigration.includes("workspace_origin in ('allo', 'import')") && importedWorkspaceLifecycleMigration.includes("workspace_status = 'active'") && importedWorkspaceLifecycleMigration.includes("next_workflow_status") && importedWorkspaceLifecycleMigration.includes("0026_imported_workspace_lifecycle"));
check("imported workspace lifecycle rollback is scoped and evidence-backed", importedWorkspaceLifecycleRollback.includes("imported_workspace_reclassified") && importedWorkspaceLifecycleRollback.includes("0026_imported_workspace_lifecycle") && !importedWorkspaceLifecycleRollback.includes("drop schema"));
check(
  "staff profiles separate editable preferences from organization identity",
  staffProfilesMigration.includes("preferred_name")
    && staffProfilesMigration.includes("profile_version")
    && staffProfilesMigration.includes("workspace_members_profile_fields_check")
    && workspaceMembers.includes("profile_updated")
    && workspaceMembers.includes("expectedVersion"),
);
check("staff profile rollback is scoped to migration 0027", staffProfilesRollback.includes("profile_version") && staffProfilesRollback.includes("0027_staff_profiles") && !staffProfilesRollback.includes("drop schema"));
check(
  "workspace roster retirement is explicit, audited, and history preserving",
  workspaceRosterMigration.includes("provisional:allo:andrew-dominici")
    && workspaceRosterMigration.includes("provisional:allo:lily-florian")
    && workspaceRosterMigration.includes("provisional:allo:lorena-renaud")
    && workspaceRosterMigration.includes("provisional:allo:marta")
    && workspaceRosterMigration.includes("workspace_member_deactivated")
    && !workspaceRosterMigration.includes("update pipeline.referrals")
    && workspaceRosterMigration.includes("0028_workspace_roster"),
);
...
check(
  "imported historical workspaces are closed without inventing outcomes",
  historicalWorkspaceArchiveMigration.includes("workspace_origin in ('allo', 'import')")
    && historicalWorkspaceArchiveMigration.includes("workspace_status = 'historical'")
    && historicalWorkspaceArchiveMigration.includes("historical_outcome")
    && historicalWorkspaceArchiveMigration.includes("not_recorded")
    && historicalWorkspaceArchiveMigration.includes("historical_workspace_closed")
    && historicalWorkspaceArchiveMigration.includes("0029_historical_workspace_archive"),
);
check(
  "historical workspace archive rollback is evidence-backed and scoped to migration 0029",
  historicalWorkspaceArchiveRollback.includes("historical_workspace_closed")
    && historicalWorkspaceArchiveRollback.includes("before_workspace_status")
...
check("extraction evidence geometry is optional, normalized, and database constrained",
  extractionEvidenceBoundingBoxesMigration.includes("referral_fields_evidence_bbox_check")
    && extractionEvidenceBoundingBoxesMigration.includes("extraction_candidates_evidence_bbox_check")
    && extractionEvidenceBoundingBoxesMigration.includes("between 0 and 1")
    && extractionEvidenceBoundingBoxesMigration.includes("0032_extraction_evidence_bounding_boxes"));
check("extraction evidence geometry rollback is scoped to migration 0032",
  extractionEvidenceBoundingBoxesRollback.includes("evidence_bbox")
    && extractionEvidenceBoundingBoxesRollback.includes("0032_extraction_evidence_bounding_boxes")
    && !extractionEvidenceBoundingBoxesRollback.includes("drop schema"));
check("workflow continuity is an explicit per-user workspace state",
  workflowContinuityMigration.includes("workflow_continuity")
    && workflowContinuityMigration.includes("user_workspace_state_state_kind_check")
    && workflowContinuityMigration.includes("0033_workflow_continuity"));
check("workflow continuity rollback is scoped to migration 0033",
  workflowContinuityRollback.includes("state_kind = 'workflow_continuity'")
    && workflowContinuityRollback.includes("0033_workflow_continuity")
    && !workflowContinuityRollback.includes("drop schema"));
check("contact directory is versioned, reusable, and referral-linked",
  contactDirectoryMigration.includes("create table if not exists pipeline.contacts")
    && contactDirectoryMigration.includes("create table if not exists pipeline.referral_contacts")
    && contactDirectoryMigration.includes("unique (referral_id, contact_id, role)")
    && contactDirectoryMigration.includes("referral_contacts_one_primary_schedule_idx")
    && contactDirectoryMigration.includes("0034_contact_directory"));
check("contact directory rollback is scoped to migration 0034",
  contactDirectoryRollback.includes("pipeline.referral_contacts")
    && contactDirectoryRollback.includes("pipeline.contacts")
    && contactDirectoryRollback.includes("0034_contact_directory")
    && !contactDirectoryRollback.includes("drop schema"));
check(
...
check("production seed requires the latest migration", productionSeed.includes("0034_contact_directory") && productionSeed.includes("migrations.length !== 34"));
check("live database smoke requires the latest migration", liveSmoke.includes("0034_contact_directory") && liveSmoke.includes("migrations.length === 34") && liveSmoke.includes("workflow_continuity") && liveSmoke.includes("pipeline.contacts") && liveSmoke.includes("pipeline.client_update_outbox"));
