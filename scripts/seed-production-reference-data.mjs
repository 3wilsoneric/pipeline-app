...
try {
  const result = await sql.begin(async (tx) => {
    ...
    `;
    if (migrations.length !== 34) throw new Error("missing_migrations");
    const rows = await tx`
      insert into pipeline.store_revisions (store_name)
      values ('referrals'), ('assessments'), ('resident_links'), ('workflow'), ('documents'), ('extraction_jobs'), ('client_workspaces'), ('client_file_imports'), ('contacts')
