...
async function requireCurrentSchema() {
  const rows = await sql`
    ...
    where migration_id between '0001_pipeline_core' and '0028_workspace_roster'
  `;
  check("latest migration set is available", Number(rows[0].count) === 28, { migrations: Number(rows[0].count) });
}
