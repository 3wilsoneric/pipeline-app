...
check("metric names and values are validated", metrics.includes("Number.isFinite(value)") && metrics.includes("/^[a-z][a-z0-9_.]{2,80}$/"));
...
const allowedDimensionBlock = metrics.slice(
  metrics.indexOf("const allowedDimensions"),
  metrics.indexOf("]);", metrics.indexOf("const allowedDimensions")) + 3,
);
check(
...
check("metric dimensions are capped", metrics.includes(".slice(0, 8)"));
