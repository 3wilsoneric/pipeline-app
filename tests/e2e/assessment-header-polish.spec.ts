...
async function createHeaderAssessment(page: Page) {
  ...
  const created = await page.request.post(`/api/referrals/${referral.id}/assessments`, { data: {
    client_mutation_id: randomUUID(), data: {
      diagnosis_categories: ["schizoaffective"], current_symptoms: "Synthetic documented symptoms.", cognition_orientation: "Synthetic orientation note.",
  ...
  return { referral, url: `/?view=referrals&screen=packet&referralId=${referral.id}&workspaceStage=assessment&assessmentSection=diagnosis_clinical` };
}
...
for (const width of [1440, 1024, 834, 768, 640]) {
  test(`assessment header keeps status legible and navigation usable at ${width}px`, async ({ page }, info) => {
    ...
    await expect(header).toContainText("Section 7 of 12");
    await expect(status).toHaveText("3 unanswered");
    ...
    if (width >= 760 && width <= 959) {
      ...
      expect(count.y + count.height).toBeLessThanOrEqual(title.y);
    }
...
test("review warnings remain distinct from unanswered questions", async ({ page }) => {
  ...
  await page.goto(url);
  const header = page.getByRole("navigation", { name: "Assessment sections", exact: true });
  await expect(header.getByRole("status")).toHaveText("1 unanswered · 1 to verify");
...
for (const width of [390, 320]) {
  test(`phone assessment retains its compact header at ${width}px`, async ({ page }) => {
    ...
    await expect(interview).toContainText("Section 5 of 12");
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  });
}
...
test("iPad WebKit retains the native section picker and readable status", async ({ baseURL }, info) => {
  ...
  try {
    ...
    await page.goto("/?view=referrals&screen=packet&trainingAssessment=prepare&demo=1&workspaceStage=assessment&assessmentSection=prior_history");
    const header = page.getByRole("navigation", { name: "Assessment sections", exact: true });
    await expect(header.getByRole("status")).toHaveCSS("font-size", "16px");
    await header.getByRole("combobox", { name: "Assessment section", exact: true }).selectOption("legal_conservatorship");
    await expect(header).toContainText("Section 10 of 12");
    expect(await header.evaluate((el) => el.scrollWidth <= el.clientWidth)).toBe(true);
