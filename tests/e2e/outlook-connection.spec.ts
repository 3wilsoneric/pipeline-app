...
async function disconnectedOutlook(page: Page) {
  // A deliberately disconnected browser never opens Microsoft silently or interactively.
  await page.addInitScript(({ clientId, email }) => localStorage.setItem(`pipeline.outlook.disconnected:${clientId}:${email}`, "true"), { clientId, email: setup.account_email });
...
for (const width of [1440, 834, 390, 320]) test(`Outlook is offered before a handoff with a dismissible prompt at ${width}px`, async ({ page, context }, info) => {
  ...
  await disconnectedOutlook(page);
  let microsoftRequests = 0;
  page.on("request", request => { if (request.url().includes("login.microsoftonline.com")) microsoftRequests++; });
  ...
  await disconnectedOutlook(nextVisit);
