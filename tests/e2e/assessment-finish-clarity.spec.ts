...
for (const width of [1440, 834, 390]) {
  test(`review, signature, decision and email remain distinct at ${width}px`, async ({ page }, info) => {
    ...
    await expect(signature).toContainText("You can still edit it until Meet the Client is sent. Changes are logged.");
    ...
    await expect(email.getByRole("button", { name: "Review handoff", exact: true })).toBeInViewport();
    ...
    await expect(composer.getByRole("status").filter({ hasText: "Not production yet" })).toHaveText("Not production yet — no draft will be created and no email will be sent.");
