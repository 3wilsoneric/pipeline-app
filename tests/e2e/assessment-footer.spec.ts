...
for (const width of [1440, 768, 390, 320]) {
  test(`assessment footer follows preparation, interview and decision at ${width}px`, async ({ page }, info) => {
    ...
    await expect(begin.getByRole("button", { name: "Keep preparing", exact: true })).toBeFocused();
    await page.keyboard.press("Tab");
    await expect(begin.getByRole("button", { name: "Begin assessment", exact: true })).toBeFocused();
...
test("secondary actions close on Escape and outside press without exiting the assessment in WebKit", async ({ baseURL }) => {
  ...
  try {
    ...
    await expect(surface.getByText("Practice changes saved locally", { exact: true })).toBeVisible();
