...
test.describe("admission follow-through", () => {
  ...
  for (const width of [1440, 390]) test(`planned dates, confirmation and reopening persist at ${width}`, async ({ browser, baseURL }, info) => {
    ...
    try {
      ...
      await admission.getByLabel("Actual admission date", { exact: true }).fill(calendarToday());
      page.once("dialog", (dialog) => dialog.dismiss());
      await admission.getByRole("button", { name: "Confirm admitted" }).click();
      ...
      const confirmation = page.waitForResponse((response) => response.url().endsWith(`/referrals/${referral.id}/transition`) && response.request().method() === "POST");
      await admission.getByRole("button", { name: "Confirm admitted" }).click();
      ...
      const replay = await api.post(`/api/referrals/${referral.id}/transition`, { data: response.request().postDataJSON() });
      expect(replay.status()).toBe(200);
      expect((await replay.json()).referral.version).toBe(confirmed.version);
      const stale = await api.post(`/api/referrals/${referral.id}/transition`, { data: { ...response.request().postDataJSON(), target_stage: "Assessment", client_mutation_id: randomUUID() } });
