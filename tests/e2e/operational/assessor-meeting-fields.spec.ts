...
test.describe("assessor meeting fields", () => {
  ...
  test("injection timing, acknowledgement, and triggers persist through the working assessment", async ({ page, baseURL }) => {
    ...
    try {
      ...
      await find("Triggers and what helps");
      const triggers = editor.getByRole("textbox", { name: "Triggers and what helps", exact: true });
      await triggers.fill("Synthetic: loud shared spaces; a quiet room helps.");
      await triggers.blur();
      await expect.poll(async () => (await read()).triggers).toBe("Synthetic: loud shared spaces; a quiet room helps.");
      ...
      await search.fill("Aggression risk");
  ...
  test("blank follow-ups allow signing and retired aggression data survives without appearing in the report", async ({ baseURL }) => {
    ...
    try {
      ...
      const response = await api.patch(`/api/assessments/${assessment.assessment_id}`, { data: {
        ...
        patch: { data: { im_injections: "yes", physical_altercations: "yes", substance_abuse_history: "yes", substance_use_insight: "partially", aggression_risk: "Synthetic preserved historical answer" } },
      } });
