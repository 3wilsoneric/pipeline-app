...
- Ordinary structured views: up to 1240px. Client folders use up to 1800px with 16–32px outer gutters in one full-width column at every viewport, with 24px between folders. From 768px, use three rows of four summary fields, giving the first column twice the width of the others; below that, use a two-column summary. Search and filters align with the folders. The compact List view remains a separate, denser alternative.
...
Cards add two rows below the placement summary: birth date, age, resident number and payor; then primary diagnosis, physician, diet and length of stay. Use only validated fields from the already-fetched, authorized current census, preserving missing values as dashes and numeric zero as a value. Do not infer clinical facts or fetch individual charts to populate the directory. Long values wrap without truncation. These extra fields do not expand the compact List view.
