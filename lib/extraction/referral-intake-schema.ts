 ...
 * from the extraction model. Retired aggression-risk data is retained in stored
...
> = assessmentToolFieldDefinitions
  .filter((definition) => !assessmentContextFields.has(definition.key) && definition.key !== "aggression_risk")
  .map((definition) => ({
