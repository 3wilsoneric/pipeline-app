...
function reader(archive: Archive) {
  ...
  return (sheet: string, address: string): string | number | boolean => {
    ...
    switch (cell.getAttribute("t")) {
      ...
      case "b": return raw === "1";
      case "e": throw new Error(`Fix the Excel error in ${sheet}!${address}.`);
...
export async function exportAssessmentWorkbook(template: Uint8Array, identity: WorkbookIdentity, data: AssessmentToolData) {
  ...
  for (const section of assessmentWorkbookLayout) {
    ...
    for (const field of section.fields) {
      const value = displayValue({ ...field, sheet: section.sheet }, data[field.key]);
      ...
      for (let c = 0; c < field.chunks; c++) {
        let chunk: string | number = value.slice(c * workbookChunkLength, (c + 1) * workbookChunkLength);
        if (c === 0 && typeof data[field.key] === "number") chunk = data[field.key] as number;
        if (c === 0 && field.value_type === "date" && value) chunk = Math.round((Date.parse(`${value}T00:00:00Z`) - Date.UTC(1899, 11, 30)) / 86400000);
        setCell(doc, `C${field.row + c}`, chunk);
        const row = elements(doc, "row").find((r) => r.getAttribute("r") === String(field.row + c));
        if (row) {
          if (c > 0) row.setAttribute("hidden", chunk ? "0" : "1");
...
function parseAnswer(field: typeof assessmentWorkbookFields[number], raw: string | number | boolean, baseline: AssessmentToolData[AssessmentToolFieldKey]) {
  ...
  const choice = (v: string) => {
    ...
    if (!options) return v;
    const match = options.find((o) => o.label.toLowerCase() === v.toLowerCase() || o.value === v);
...
export async function importAssessmentWorkbook(bytes: Uint8Array, identity: WorkbookIdentity): Promise<AssessmentWorkbookCopy> {
  ...
  for (const [index, field] of assessmentWorkbookFields.entries()) {
    ...
    if (field.value_type === "reason_map") continue;
  ...
  return { ...identity, exportId: meta.exportId, exportedAt: meta.exportedAt, baseline, answers };
}
