...
const intakeRoles = writeRoles;
const assessmentRoles = writeRoles;
const leadRoles = ["admin", "assessment_coordinator"] as const;
const decisionRoles = ["admin"] as const;
...
export const operatorJobAids: readonly OperatorJobAid[] = [
  jobAid("new-referral", "Create a referral safely", "When a genuinely new inbound referral arrives.", intakeRoles, "New referral", "/?view=referrals&screen=packet", "components/pipeline/ReferralPacketCanvas.tsx", ["Search for existing work", "Verify new versus update", "For the same client, use the chart New referral action when available to create a saved Intake", "Review carried-forward facts and sources; complete fresh packet approvals, signatures, and decisions", "Enter source-backed identity and received date", "Set community, source, priority, and owner", "Save changes once and verify the workspace"], ["Identity conflict", "Possible duplicate", "No accountable owner"]),
  ...
  jobAid("assessment-ready", "Prepare an assessment", "Before assigning or scheduling assessment work.", writeRoles, "Referral workspace", "/?view=referrals", "components/pipeline/ReferralPacketCanvas.tsx", ["Confirm identity", "Confirm packet review", "Confirm medication context", "Resolve or assign blockers", "Assign assessor", "Schedule with owner, time, and method"], ["Unresolved identity", "Missing required source", "No assigned assessor"]),
...
export const operatorCapabilities: readonly OperatorCapability[] = [
  ...
  capability("readiness", "Pre-assessment readiness", "Resolve blockers, ownership, medications, and scheduling before assessment.", "Coordinator", "Referral workspace", "/?view=referrals", "components/pipeline/ReferralPacketCanvas.tsx", ["packet"], ["calendar", "assessment"], ["preassessment-checklist", "assign-assessor", "schedule-assessment"]),
  ...
  capability("profiles", "Client profiles", "Use Cards or List to open the same confirmed client chart shown in a workspace, within your access; similar names do not confirm identity.", "All users", "Clients", "/?screen=profiles", "components/pipeline/ClientProfileDirectory.tsx", ["confirmed-link"], [], ["navigation-model", "duplicates-identity"]),
  capability("operations", "Operations", "Review scoped client community, referral source, care needs, and chart completeness reports alongside workflow exceptions; check coverage notes before interpreting or exporting totals.", "Supervisor / admin", "Operations", "/?screen=operations", "components/pipeline/OperationsDashboard.tsx", ["workspace", "assessment", "ehr"], ["supervisor-action"], ["dashboard-meaning", "supervisor-exceptions"]),
];
