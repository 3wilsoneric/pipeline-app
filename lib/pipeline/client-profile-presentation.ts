...
const profileSections: SectionDefinition[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { label: "Prior setting type", sources: ["prior_setting_bucket", "prior_setting_enriched_json", "operational_prior_setting_json", "structured_prior_setting_buckets_json", "previous_living_status_values_json"], maxItems: 6 },
      { label: "Prior placements", sources: ["prior_placements"], maxItems: 8 },
      { label: "Prior placement name", sources: ["referring_facility", "referring_facilities_enriched_json", "operational_referring_facilities_json", "recovered_document_referring_json", "document_gap_referring_facilities_json"], maxItems: 6 },
...
export function describeClientStayCount(
  ...
  if (episodes.length === 0 && history.status !== "available") {
    return {
      label: history.status === "not_found" ? "No stays found in placement history"
        : history.status === "identity_conflict" ? "Stay history needs identity review" : "Stay history unavailable",
      note: null,
    };
  }
  ...
  const label = total === 0 ? "No recorded stays"
