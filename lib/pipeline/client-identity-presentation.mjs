...
// normalization fallback, and this module's own unnamed-referral label. Never
...
export function formatUnnamedReferralTitle(referralId) {
  return `Unnamed referral · #${referralId}`;
}

export function presentClientName(value, referralId) {
  return displayClientName(value) || missingNameLabel(referralId);
}
...
export function formatClientIdentityTitle({ name, gender, community, referralId, id }) {
  ...
  return displayClientName(name, { community })
    || missingNameLabel(referralId ?? (typeof id === "number" ? id : undefined));
}

// Secondary line that keeps referral rows distinguishable: a named referral
// gets its ID (two people can share a name); an unnamed one already shows its
// ID in the title, so it gets received-date and source context instead.
...
export function formatReferralIdentityContext({ name, referralId, received, source }) {
  if (!Number.isSafeInteger(referralId) || referralId <= 0) return "";
  if (displayClientName(name)) return `Referral #${referralId}`;
...
function missingNameLabel(referralId) {
  return Number.isSafeInteger(referralId) && referralId > 0
    ? formatUnnamedReferralTitle(referralId)
...
export function normalizeClientName(value, options = {}) {
  ...
  return normalizeNameCasing(firstAndLastName(name));
}
