...
function validatePlannedMaterial(material) {
  ...
  validatePlannedMaterialLocation(material);
}

function validatePlannedMaterialLocation(material) {
  if (material.source_path != null) {
    if (!path.isAbsolute(String(material.source_path))) throw new Error("historical_simulation_material_path_invalid");
    return;
  }
  const hasMaterializedObject = /^objects\/sha256\/[a-f0-9]{2}\/[a-f0-9]{64}$/.test(String(material.object_relpath ?? ""));
  if (material.source_available && !hasMaterializedObject) throw new Error("historical_simulation_material_path_invalid");
}
