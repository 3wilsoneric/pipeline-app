...
export async function GET(request: Request) {
  return withApiLogging(request, "/api/operations/dashboard", async () => {
    ...
    if (!canAccessOperationsReports(auth.user)) return jsonError("Reports are not available for this account.", 403);
    const month = new URL(request.url).searchParams.get("month") ?? new Date().toISOString().slice(0, 7);
    if (!/^(?:20|21|22)\d{2}-(?:0[1-9]|1[0-2])$/.test(month)) return jsonError("month must use YYYY-MM.");
    const { snapshot, supervisorQueue } = await getOperationsDashboardSnapshot(auth.user, true, month);
