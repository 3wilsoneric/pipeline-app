...
async function handle(request: Request, context: Context, mutate: boolean) {
  return withApiLogging(request, "/api/referrals/[referralId]/admission-packets", async () => {
    ...
    try {
      ...
      return json(await manageAdmissionPacketLink(packet_id, referralId, action, pipelineAccountableActor(auth.user)));
    } catch (error) { return json({ error: error instanceof PacketAccessError ? error.message : "Packet access settings could not be loaded. Try again." }, error instanceof PacketAccessError ? error.status : 503); }
  });
}
