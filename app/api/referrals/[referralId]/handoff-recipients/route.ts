...
type RecipientDraft = RecipientFields & { community: string };
...
export async function GET(request: Request, context: Context) {
  return withApiLogging(request, "/api/referrals/[referralId]/handoff-recipients", async () => {
    ...
    if (draft && (!parseRecipientFields(draft) || typeof draft.community !== "string")) return jsonError("The saved recipient draft could not be read.", 409);
...
export async function PUT(request: Request, context: Context) {
  return withApiLogging(request, "/api/referrals/[referralId]/handoff-recipients", async () => {
    ...
    if (!access.ok) return access.response;
    const body = await readJsonBody<{ if_match?: unknown; draft?: RecipientDraft }>(request, 64_000);
    ...
    const result = await putUserWorkspaceState({ principalId: access.user.id, kind: "referral_email_draft", key: access.key,
      payload: { ...fields, community: access.referral.community }, expectedVersion: Number(body.value.if_match), ttlDays: 30 });
